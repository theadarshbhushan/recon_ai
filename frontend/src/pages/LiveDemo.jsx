...
import { Zap, CheckCircle2, AlertTriangle, ShieldCheck, Sparkles } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-wrap gap-3">
          {PRESETS.map((p, idx) => (
            <button
              ...
              className="rounded-full px-5 py-2 text-xs font-black bg-slate-100 hover:bg-slate-200/80 border border-slate-200 text-slate-800 transition-all cursor-pointer shadow-2xs hover-scale"
            >
              {p.name}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-xs">
          <h3 className="wise-card-title text-base mb-4 pb-3 border-b border-slate-100">
          ...
          <form onSubmit={handleRunInference} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-900 focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 focus:outline-none"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 focus:outline-none"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-4 rounded-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-black text-sm py-3.5 px-6 shadow-md hover:shadow-lg hover:shadow-indigo-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer hover-scale"
            >
              {loading ? (
                <>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
              <h3 className="wise-card-title text-base">Model Verdict & Audit Explanation</h3>
              {result && (
            ...
            {result ? (
              ...
              <div className="text-center py-16 text-slate-400 text-xs">
                <Zap className="h-8 w-8 mx-auto mb-2 opacity-40 text-indigo-600" />
