              ...
              {benchmarkData.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-3.5 px-4 font-black text-slate-900 flex items-center gap-2">
                    {idx === 0 ? <Zap className="h-4 w-4 text-blue-600" /> : <Bot className="h-4 w-4 text-slate-400" />}
                    <span>{row.model}</span>
                  </td>
                  ...
                  <td className="py-3.5 px-4 text-center font-black font-mono text-blue-600">
                    {(row.f1).toFixed(4)}
                  </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white border border-slate-200/90 p-6 sm:p-8 rounded-2xl shadow-xs">
          <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-100">
            <div>
              <h3 className="wise-card-title text-base text-slate-900">Inference Latency (Milliseconds)</h3>
        ...
        <div className="bg-white border border-slate-200/90 p-6 sm:p-8 rounded-2xl shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
              <span className="wise-eyebrow text-blue-600">Research Validation</span>
              <span className="rounded-full bg-blue-50 text-blue-800 border border-blue-200 px-3 py-0.5 text-xs font-black">
                Foundation Model
              </span>
            </div>
            <h3 className="wise-card-title text-base text-slate-900 mb-3">
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-500">
            <span>Production Choice: <strong className="text-slate-900">CatBoost</strong></span>
