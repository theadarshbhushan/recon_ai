...
const KpiCard = ({ title, value, borderClass = "border-l-[#0A2540]", subtext, subtextColor = "text-slate-500" }) => {
  return (
    <div className={`bg-white border border-slate-200/90 p-6 rounded-2xl shadow-xs hover:shadow-md transition-all duration-200 border-l-4 ${borderClass} flex flex-col justify-between`}>
      <div>
        ...
        <div className="text-3xl sm:text-4xl font-black text-[#0A2540] tracking-tight">
          {value}
        </div>
      </div>
      {subtext && (
